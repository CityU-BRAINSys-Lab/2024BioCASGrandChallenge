...
# Results are saved to results_maxR2.json and results_tinyRSNN.json
...
# LOCAL
from challenge.utils.snntorch import get_maxR2_model, get_tinyRSNN_model
...
# SET UP LOGGER
logging.basicConfig()
logger = logging.getLogger("train-maxR2")
...
class BenchmarkResultsHandler:
    ...
    def get_summary(self):
        return {
            ...
            'connection_sparsity': sum(self.connection_sparsity) / len(self.connection_sparsity),
            'activation_sparsity': sum(self.activation_sparsity) / len(self.activation_sparsity),
...
def load_and_benchmark_stork_model(cfg, 
                                   modelname,
                                   state_dict_dir, 
                                   train_dat, 
                                   test_dat,
    ...
    if modelname == 'maxR2':
        this_cfg.model = cfg.maxR2
        this_cfg.training = cfg.maxR2training
    elif modelname == 'tinyRSNN':
        this_cfg.model = cfg.tinyRSNN
        this_cfg.training = cfg.tinyRSNNtraining
    else:
        raise ValueError("Invalid modelname. Must be 'maxR2' or 'tinyRSNN'")
...
def benchmark_all_models_stork(cfg, modelname, state_dict_paths, train_dat, test_dat, monkey_name, dtype):
    """
    ...
    for idx, state_dict_path in enumerate(state_dict_paths):
        ...
        _, bm_results = load_and_benchmark_stork_model(cfg, 
                                                       modelname,
                                                       state_dict_path,
                                                       train_dat, test_dat, 
                                                       monkey_name, 
        ...
        logger.info("MACs: {}".format(bm_results['synaptic_operations']['Effective_MACs']))
        logger.info("ACs: {}".format(bm_results['synaptic_operations']['Effective_ACs']))
...
def load_data_snnTorch(cfg, filename, dtype):
    ...
    # Load the dataset
    dataset = PrimateReaching(file_path=cfg.data.data_dir, filename=filename,
                            num_steps=1, train_ratio=0.5, bin_width=4e-3,
...
def load_snnTorch_model(cfg, modelname, state_dict_dir, device):
    ...
    if modelname == 'maxR2':
        logger.info("Creating maxR2 snnTorch model...")
        model = get_maxR2_model(cfg.maxR2,
                                torch.load(state_dict_dir),
                                device)
    
    elif modelname == 'tinyRSNN':
        logger.info("Creating tinyRSNN snnTorch model...")
        model = get_tinyRSNN_model(cfg.tinyRSNN,
                                   torch.load(state_dict_dir),
                                   device)
    
    return model

def load_and_benchmark_snnTorch_model(cfg, 
                                      modelname,
                                      state_dict_path, 
                                      test_set_loader, 
                                      dtype):
    ...
    benchmark = Benchmark(BMmodel, 
                        test_set_loader, 
                        [], [], 
                        [cfg.static_metrics, cfg.workload_metrics])
...
def benchmark_all_models_snnTorch(cfg, modelname, state_dict_paths, test_set_loader, dtype):
    """
    Benchmark all models in the given state_dict_paths
    """
    ...
    for idx, state_dict_path in enumerate(state_dict_paths):
        ...
        # Load and benchmark the model
        _, bm_results = load_and_benchmark_snnTorch_model(cfg, 
                                                          modelname,
                                                          state_dict_path,
                                                          test_set_loader,
                                                          dtype)
...
def evaluate(cfg: DictConfig) -> None:
    ...
    logger.info("Evaluating {} models in the {} directory".format(cfg.modelname, cfg.model_dir))
    ...
    if cfg.modelname == 'all':
        cfg.modelname = ['maxR2', 'tinyRSNN']
    elif cfg.modelname in ['maxR2', 'tinyRSNN']:
        cfg.modelname = [cfg.modelname]
    else:
        raise ValueError("Invalid model type. Must be 'maxR2', 'tinyRSNN', or 'all'")
    ...
    for monkey_name in ['loco', 'indy']:
        ...
        for session, filename in cfg.data.filenames[monkey_name].items():
            ...
            for model_type in cfg.modelname:
                ...
                state_dict_paths = [Path(f) for f in model_dir.iterdir() if model_type in f.name]
                logger.info("Found {} {} models for session {}".format(len(state_dict_paths), model_type, session))
                ...
                if cfg.use_snnTorch_model:
                    ...
                    train_dat, _, test_dat = storkmodel_dataloader.get_single_session_data(filename)
                ...
                with open(session_results_save_path / 'results_{}_{}.json'.format(model_type, filename), 'w') as f:
                    json.dump(session_results.to_dict(), f, indent=4)
                logger.info("Saved all session results to results_{}_{}.json".format(model_type, filename))
    ...
    for model_type in cfg.modelname:
        ...
        for metric in ['footprint', 'connection_sparsity', 'activation_sparsity', 'dense', 'macs', 'acs', 'r2']:
            results[model_type]['average'][metric] = sum([results[model_type][file][metric] for file in all_session_filenames]) / len(all_session_filenames)
        ...
        with open(result_save_path / 'results_summary_{}.json'.format(model_type), 'w') as f:
...
# maxR2 (results_summary_maxR2.json)
# # # # # # # # # # # # # # # # # # # # #
