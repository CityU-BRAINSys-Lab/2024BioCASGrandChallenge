# Trains `MaxR2` model on all sessions of both monkeys.
# # # # # # # # # 
...
@hydra.main(config_path="conf", config_name="train-maxR2", version_base="1.1")
    ...
    for monkey_name in ["loco", "indy"]:
        ...
        if cfg.pretraining:
            ...
            model, history = train_validate_model(
                ...
                snapshot_prefix="maxR2_pretrain_" + monkey_name + "_",
            )
            results = {}
            ...
            with open("maxR2-results-pretraining-" + monkey_name + ".json", "w") as f:
                json.dump(converted_results, f, indent=4)
        
            # Save pretrained model state
            save_model_state(model, "maxR2-pretrained-" + monkey_name + ".pth")
        ...
        for session_name, filename in cfg.data.filenames[monkey_name].items():
            ...
            model, history = train_validate_model(
                ...
                verbose=cfg.training.verbose,
                snapshot_prefix="maxR2_" + session_name + "_",
            )
            ...
            save_model_state(model, "maxR2-" + session_name + ".pth")

            # If seed is set, save model state as 'models / {session_name} / maxR2-{seed}.pth'
            if cfg.seed:
                path = Path(to_absolute_path('models')) / session_name
                path.mkdir(parents=True, exist_ok=True)
                filepath = path / ("maxR2-" + str(cfg.seed) + ".pth")
            ...
            if cfg.plotting.plot_cumulative_mse:
                fig, ax = plot_cumulative_mse(
                    model, val_dat, save_path="maxR2_cumulative_se_" + session_name + ".png"
                )
            ...
            # Save to JSON file with indentation
            converted_results = convert_np_float_to_float(results)
            with open("maxR2-results-" + session_name + ".json", "w") as f:
                json.dump(converted_results, f, indent=4)
            ...
            if cfg.plotting.plot_training:
                fig, ax = plot_training(
                    ...
                    cfg.training.nb_epochs_train,
                    names=["loss", "r2"],
                    save_path="maxR2_training_" + session_name + ".png",
                )
