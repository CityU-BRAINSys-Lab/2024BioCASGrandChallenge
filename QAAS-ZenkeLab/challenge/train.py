...
class DensityBasedWeighting():
    ...
    def eval(self, y):
        ...
        return torch.from_numpy(rels.reshape(shape))
...
def _choose_loss(cfg):
    ...
    if cfg.training.loss == "MSE":
        loss_class = stork.loss_stacks.MeanSquareError
    elif cfg.training.loss == "RMSE":
        loss_class = stork.loss_stacks.RootMeanSquareError
    elif cfg.training.loss == "MAE":
        loss_class = stork.loss_stacks.MeanAbsoluteError
    elif cfg.training.loss == "Huber":
        loss_class = stork.loss_stacks.HuberLoss
    else:
...
def get_train_loss(cfg, train_data):
    ...
    # Weighted loss
    if cfg.training.density_weighted_loss:        
        args['density_weighting_func'] = DensityBasedWeighting(
            y=np.concatenate([y for _, y in train_data]).flatten(),
            alpha=cfg.training.density_weighting_alpha
        )
...
def train_validate_model(
    ...
    # Check if the number of timesteps is the same in the training and validation data
    if train_data[0][0].shape[0] != valid_data[0][0].shape[0]:
        n_timesteps_train = train_data[0][0].shape[0]
        n_timesteps_val = valid_data[0][0].shape[0]
    else:
    ...
    history, _, _ = model.fit_validate(
        train_data,
        valid_data,
        n_timesteps_train=n_timesteps_train,
        n_timesteps_val=n_timesteps_val,
        nb_epochs=nb_epochs,
        verbose=verbose,
        mask=mask,
    )
