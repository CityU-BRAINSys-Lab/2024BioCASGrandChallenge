...
def get_maxR2_model(model_cfg, stork_state_dict, device):
    """
    Build an equivalent snnTorch model of the MaxR2 model
    ...
    # Get initialization arguments for MaxR2 model
    ...
    for idx in range(len(tau_syn_readouts)):
        SNNtorch_state_dict[f'fc_outputs.{idx}.weight'] = stork_state_dict[f'con{idx+3}.op.weight']
...
class maxR2(nn.Module):
    """
    snnTorch equivalent of the MaxR2 model
    """
    ...
    def __init__(self, 
        ...
        for i in range(self.nb_readout_heads):
            self.lif_readouts.append(Synaptic_storklike(
                tau_syn=torch.nn.Softplus()(syn_param_readouts[i]) * tau_syn_readouts[i],
                tau_mem=torch.nn.Softplus()(mem_param_readouts[i]) * tau_mem_readouts[i],
                dt = dt,
                spike_grad=self.surrogate, 
                threshold=1, 
                learn_beta=False,
            ...
            self.fc_outputs.append(nn.Linear(self.hidden_size,
                                             self.output_size,
                                             bias=False, 
                                             device=device))
        ...
        self.mem_readouts, self.syn_readouts = [None]*self.nb_readout_heads, [None]*self.nb_readout_heads
        self.out = None
    ...
    def reset(self):
        ...
        for i in range(self.nb_readout_heads):
            self.mem_readouts[i], self.syn_readouts[i] = self.lif_readouts[i].init_synaptic()
