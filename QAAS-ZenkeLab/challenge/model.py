...
from stork.nodes import (
    InputGroup,
    ReadoutGroup,
    TsodyksMarkramLearnSTP,
    AverageReadouts,
    LinearInstantReadoutGroup,
)
...
def get_model(cfg, nb_inputs, dtype, data=None):
    ...
    if cfg.filter.apply_filter:
        filter_params = {
            "cutoff": cfg.filter.cutoff,
            "sampling_rate": 1 / cfg.data.dt,
        }
    else:
        filter_params = None
    ...
    if cfg.model.use_stp:
        current_src_grp = model.add_group(
            TsodyksMarkramLearnSTP(
                current_src_grp,
                tau_d_max=cfg.model.stp_tau_d_max,
                tau_f_max=cfg.model.stp_tau_f_max,
                U=cfg.model.stp_U,
                learn=True,
            )
        )
...
def get_custom_readouts(cfg):
    ...
    for ro, specs in cfg.model["readouts"].items():
        ...
        if specs["type"] == "default":
            ...
            ro_group = LinearInstantReadoutGroup(cfg.data.nb_outputs)
